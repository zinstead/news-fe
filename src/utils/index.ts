...
export const convertLabelToTitle = (data: TreeNode[]): ConvertedNode[] =>
  data.map(({ label, children, ...rest }) => ({
    ...rest,
    title: label,
...
interface UserTokenType {
  ...
  roleState: boolean;
  default: boolean;
  role: { id: number; rights: string[]; roleType: RoleType; roleName: string };
}

export const getUserToken = (): UserTokenType => {
